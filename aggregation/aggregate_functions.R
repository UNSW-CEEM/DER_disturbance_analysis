...
find_grouping_cols <- function(settings) {
    ...
    if (settings$standard_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "Standard_Version")
    }
    if (settings$pst_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "s_postcode")
    }
    if (settings$grouping_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "Grouping")
    }
    if (settings$manufacturer_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "manufacturer")
    }
    if (settings$model_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "model")
    }
    if (settings$response_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "response_category")
    }
    if (settings$zone_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "zone")
    }
    if (settings$compliance_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "compliance_status")
    }
    if (settings$compliance_2020_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "compliance_status_2020")
    }
    if (settings$reconnection_compliance_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "reconnection_compliance_status")
    }
    if (settings$v_excursion_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "voltage_excursion")
    }
    if (settings$circuit_agg==TRUE) {
        grouping_cols <- c(grouping_cols, "site_id", "c_id")
    }
...
vector_groupby_count_response <- function(data, grouping_cols) {
    if (!"response_category" %in% grouping_cols) {
        grouping_cols <- c(grouping_cols, "response_category")
    }
    grouping_cols <- grouping_cols[grouping_cols != "c_id"]
