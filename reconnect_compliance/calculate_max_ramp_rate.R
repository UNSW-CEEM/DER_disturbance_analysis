...
calculate_total_ramp_while_exceeding_ramp_rate_compliance_threshold <- function(
  normalised_power_profiles, event_time, disconnect_threshold, reconnect_threshold, ramp_rate_threshold) {
  ...
  max_ramp_rates <- group_by(normalised_power_profiles, c_id)
  total_ramp_above_threshold <- as.data.frame(summarise(max_ramp_rates, 
                                                        ramp_above_threshold = sum(ramp_above_threshold, na.rm = TRUE)))
