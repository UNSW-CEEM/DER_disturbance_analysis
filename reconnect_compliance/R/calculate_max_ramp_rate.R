...
calculate_max_reconnection_ramp_rate <- function(normalised_power_profiles, event_time, disconnect_threshold, 
  ...
  distconnected_intervals <- filter(normalised_power_profiles, c_id_daily_norm_power < disconnect_threshold)
  ...
  connected_intervals <- filter(normalised_power_profiles, c_id_daily_norm_power > reconnect_threshold)
  ...
  normalised_power_profiles <- filter(normalised_power_profiles, ts <= fully_connected_time)
  ...
  normalised_power_profiles <- filter(normalised_power_profiles, ts <= resource_limited_interval)
