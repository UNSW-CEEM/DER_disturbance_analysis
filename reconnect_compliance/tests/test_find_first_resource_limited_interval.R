...
testthat::test_that("Calculating ramp rates works",{
  ...
  first_resource_limited_interval <- dplyr::mutate(first_resource_limited_interval, 
                                                   resource_limited_interval = 
                                                     as.POSIXct(resource_limited_interval, tz = "Australia/Brisbane"))
  
  calculated_resource_limited_interval <- find_first_resource_limited_interval(ramp_rates, ramp_rate_change_threshold = -.25)
