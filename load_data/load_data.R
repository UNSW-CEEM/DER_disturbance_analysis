  ...
  if (is.na(as.character(settings$load_start_time)) | is.na(as.character(settings$load_end_time))) {
    ...
    if (difftime(end_time, start_time, units = "hours") > 2) {
      ...
      errors$warnings[[length(errors$warnings) + 1]] <- list(
        title = "Warning long time window",
...
validate_required_files <- function(errors) {
  ...
  if (!file.exists(INSTALL_DATA_FILE)) {
    long_error_message <- paste0(
      "The required file cer_cumulative_capacity_and_number.csv could not be found. ",
  ...
  if(settings$frequency_data_file != "") {
    ...
    for (col_name in column_names) {
      if (!(col_name %in% expected_columns_names)) {
        long_error_message <- paste0(
          "The frequency data CSV should only contain the following columns, ",
        ...
        errors$errors[[length(errors$errors) + 1]] <- list(
          title = "Error loading frequency data",
          body = long_error_message
        )
      }
    }
    if (!(settings$region_to_load %in% column_names)) {
      long_error_message <- paste0(
        "The frequency data CSV must contain a column for the region being loaded. ",
        "Either choose to load no frequency data or ensure it containts data for the selected region."
      )
      errors$errors[[length(errors$errors) + 1]] <- list(
        title = "Error loading frequency data",
        body = long_error_message
...
load_data <- function(data, settings) {
  ...
  # Check timeseries for errors
  start_time <- format(as.POSIXct(settings$load_start_time, tz = "Australia/Brisbane"), tz = "GMT")
  end_time <- format(as.POSIXct(settings$load_end_time, tz = "Australia/Brisbane"), tz = "GMT")
  time_series_data <- data$db$get_filtered_time_series_data(
    state = settings$region_to_load,
