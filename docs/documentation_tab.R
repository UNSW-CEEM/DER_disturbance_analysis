documentation_panel <- function() {
  panel <- mainPanel(
    ...
    div('Compliance status is assigned on a circuit basis according to the following definitions. The categories Disconnect,
         Off at t0 and not enough data are taken from the output of the response categorisation algo.'),
    ...
    tags$ol(tags$li('The resource limited interval is defined as the first interval during the reconnection period
            ...
            tags$li('Categorised as unsure if not compliant or non complaint'),
            tags$li('Categorised as NA if the circuit does not disconnect/drop to zero during the user specified event
                     window.')),
