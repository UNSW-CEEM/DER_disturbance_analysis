...
DBInterface <- R6::R6Class(
  ...
  public = list(
    ...
    build_database = function(timeseries,
      ...
      RSQLite::dbExecute(
        con,
        "UPDATE alerts SET SYNC_a038_DoOpenArguments = NULL WHERE SYNC_a038_DoOpenArguments = ''"
      )
      ...
      RSQLite::dbExecute(
        con,
        "UPDATE alerts SET SYNC_a010_vfCheckFreqWobble = NULL WHERE SYNC_a010_vfCheckFreqWobble = ''"
      )
      RSQLite::dbExecute(
        con,
        "UPDATE alerts SET SYNC_a005_vfCheckUnderVoltage = NULL WHERE SYNC_a005_vfCheckUnderVoltage = ''"
      )
    ...
    run_data_cleaning_loop = function(max_chunk_size = 500) {
      ...
      while (length(circuits$c_id) > 0) {
        ...
        pv_time_series <- filter(
          time_series,
          con_type %in% c("pv_site_net", "pv_site", "pv_inverter_net", "pv_inverter")
        )
    ...
    insert_postcode_lon_lat_cleaned = function(file_path_name) {
      ...
      # Suppress warning
      withCallingHandlers({
        invisible(sqldf::sqldf(query, dbname = self$db_path_name)) # stops an empty dataframe being printed.
      }, warning = function(w) {
        if (startsWith(conditionMessage(w), "Don't need to call dbFetch()")) {
          invokeRestart("muffleWarning")
        }
