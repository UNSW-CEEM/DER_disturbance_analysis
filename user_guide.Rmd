...
## Table of Contents
1. [Creating a sqlite database from solar analytics data](#creating-a-sqlite-database-from-solar-analytics-data)
n. [Data available post cleaing](#data-available-post-cleaing)
...
You may receive three possible errors at this stag.
...
db_two$build_database(timeseries = timeseries_path_name,
                  ...
                  site_details = site_details_path_name)
```
This is fixed by overiding the default timeseries columns, in this case the 'v' column is givem the alias 'voltage'.
...
db_two$build_database(timeseries = timeseries_path_name,
                  ...
                  site_details = site_details_path_name)
```
This is fixed by editing the site details file directly and my require adjusting the ac and dc capacity units.
...
Before running data cleaning postcode lattiude and longditude data needs to be added to the database, this is for 
calculating sunrise and sunset times that are used for validating if a pofile is from a PV system.
...
database ready for use by the tool is as follows. On data sets of 1 GB from solar analytics this takes aproximately 40 
min to run. 
```{r, warning=FALSE, message=FALSE}
timeseries_path_name <- "BDInterface/tests/testthat/data/simple_timeseries.csv"
site_details_path_name <- "BDInterface/tests/testthat/data/simple_site_details.csv"
