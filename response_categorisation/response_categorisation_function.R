...
categorise_by_response <- function(event_window_data, window_length, NED_threshold_pct) {
  event_window_data <- event_window_data %>%
    mutate(response_category = ifelse(event_power < 0.1, "5 Off at t0", "Undefined")) %>%
    # NED if sampled seconds below threshold
    mutate(
      response_category = ifelse(
        num_data_points < (60 / d) * window_length * NED_threshold_pct,
        "6 Not enough data",
        response_category
      )
    ) %>%
    # NED if first_ts not sampled
    mutate(response_category = ifelse(first_ts == "not sampled", "6 Not enough data", response_category)) %>%
    mutate(
       response_category = ifelse(
     ...
     mutate(
       response_category = ifelse(
         response_category == "Undefined" & min_norm_power < 0.96 & num_con_zeros == 0,
         "2 Curtail",
         response_category
       )
     ) %>%
