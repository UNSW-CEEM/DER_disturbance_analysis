...
get_bootstrap_confidence_interval <- function(circuits_data, manufacturer_capacitys, predictors_list, min_samples=30,
  ...
  set.seed(1)
  bootstrap_correlation <- boot(circuits_data, glm_wrapper, R=num_repetitions)
  #bootstrap_correlation <- boot(circuits_data, bucket_wrapper, R=5000)
  
  boot_ci_results <- c()
  boot_ci_results$original_kw_loss <- bootstrap_correlation$t0
  boot_ci_results$mean_kw_loss <- mean(bootstrap_correlation$t)
  boot_ci_results$lower_bound_perc <- boot.ci(boot.out=bootstrap_correlation, type="perc")$percent[4]
  boot_ci_results$upper_bound_perc <- boot.ci(boot.out=bootstrap_correlation, type="perc")$percent[5]
  return(data.frame(boot_ci_results))
}
