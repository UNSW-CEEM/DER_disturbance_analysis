...
get_upscaling_results_excluding_ufls_affected_circuits <- function(circuit_summary,
...
join_circuit_summary_and_cer_manufacturer_data <- function(circuit_summary, cer_manufacturer_data) {
  circuit_summary <- merge(
    circuit_summary,
    cer_manufacturer_data,
    by = c("Standard_Version", "manufacturer"),
    all = TRUE
  )
...
impose_sample_size_threshold <- function(disconnection_summary, sample_threshold) {
  disconnection_summary["manufacturer"][
    is.na(disconnection_summary["disconnections"]) | is.na(disconnection_summary["cer_capacity"])
  ] <- "Other"
  disconnection_summary["sample_threshold"] <- sample_threshold
  
  # Create an Other group for manufacturers with a small sample size.
  setnafill(disconnection_summary, cols = c("sample_size"), fill = 0, type = "const")
  disconnection_summary <- disconnection_summary %>%
    mutate(
      manufacturer = ifelse(
        (
...
calc_confidence_intervals_for_disconnections <- function(disconnection_summary) {
  result <- mapply(CI_wrapper, disconnection_summary$disconnections, disconnection_summary$sample_size, 0.95)
...
get_manufacturer_capacitys_variable <- function(manufacturer_install_data,
                                                event_date,
                                                region,
  ...
  manufacturer_install_data <- manufacturer_install_data %>%
    group_by_at(append(grouping_cols, 's_state')) %>%
...
upscale_disconnections <- function(manufacturer_level_summary) {
  ...
  upscaled_disconnections <- summarise(
    upscaled_disconnections,
