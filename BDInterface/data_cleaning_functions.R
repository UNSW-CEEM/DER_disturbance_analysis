calc_max_kw_per_site <- function(performance_data) {
    ...
    performance_data <- summarise(
        performance_data,
        power_kW=sum(power_kW, na.rm=TRUE)
    )
    performance_data <- as.data.frame(performance_data)
    performance_data <- group_by(performance_data, site_id)
    site_max_power <- summarise(
        performance_data,
        max_power_kW=max(power_kW, na.rm = TRUE)
    )
...
check_for_ac_value_in_watts <- function(site_details) {
    ...
    site_details <- mutate(
        site_details,
        ac=ifelse(ac_dc_ratio > 0.1 & ac > 150, ac/1000, ac)
    )
    return(site_details)
}
...
clean_connection_types <- function(
    combined_data,
    circuit_details,
    postcode_data
) {
    postcode_data <- calc_sunrise_sunset_bounds(
        postcode_data,
        as.Date(combined_data$ts[1])
    )
    ...
    combined_data <- mutate(
        combined_data,
        con_type_changed=ifelse(con_type!=old_con_type, 1, 0)
    )
    combined_data <- mutate(
        combined_data,
        polarity_changed=ifelse(polarity!=old_polarity, 1, 0)
    )
...
clac_output_summary_values <- function(combined_data) {
    ...
    combined_data <- mutate(
        combined_data,
        energy_day=ifelse(is.na(energy_day), 0.0, energy_day)
    )
    combined_data <- mutate(
        combined_data,
        energy_night=ifelse(is.na(energy_night), 0.0, energy_night)
    )
    ...
    combined_data <- mutate(
        combined_data,
        frac_day=round(energy_day/(energy_day+energy_night), digits=2)
    )
}
...
check_for_mixed_polarity <- function(combined_data) {
    ...
    combined_data <- mutate(
        combined_data,
        con_type=ifelse(
            max_power > ac * .1 & min_power * -1 > ac * 0.1,
            "mixed",
            con_type
        )
    )
...
remove_outlying_voltages <- function(time_series) {
    ...
    time_series[, voltage_cols] <- sapply(
    ...
    time_series[, voltage_cols] <- (
        time_series[, voltage_cols] *
        voltage_extremes[
            match(time_series$c_id, voltage_extremes$c_id),
            voltage_cols
        ]
    )
    time_series['v_changed'] <- rowSums(
        is.na(time_series[, voltage_cols]) &
        !is.na(old_time_series[, voltage_cols])
    ) > 0
    return(time_series)
}
