...
testthat::test_that("Building works when no column aliases are required.", {
    ...
    expected_circuit_details <- mutate(
        expected_circuit_details,
        manual_droop_compliance='Not set'
    )
    expected_circuit_details <- mutate(
        expected_circuit_details,
...
testthat::test_that(
    "Building works when there are extra headers in the ts data.",
{
    ...
    dp$connect_to_new_database("test.db")
    dp$build_database(
        timeseries=timeseries_with_extra_header,
        circuit_details=circuit_details_path_name,
        site_details=site_details_path_name,
        check_dataset_ids_match=FALSE
    )
...
testthat::test_that(
    "Test data cleaning with batch size smaller than number of circuits.",
{
    ...
    expected_site_details <- load_test_file(site_details_path_name)
    expected_site_details_cleaned <- load_test_file(
    ...
    dp$connect_to_new_database("test.db")
    dp$build_database(
        timeseries=timeseries_with_missing_durations,
        circuit_details=circuit_details_path_name,
        site_details=site_details_path_name,
        check_dataset_ids_match=FALSE
    )
    ...
    testthat::expect_equal(
        output_site_details_cleaned,
        expected_site_details_cleaned,
        tolerance=1e-6
    )
    testthat::expect_equal(output_circuit_details, expected_circuit_details)
})
...
testthat::test_that(
    "Test calculating duration values works, with batch size
...
testthat::test_that(
    "test calculating duration values works, with batch size greater than the
    number of circuits",
{
...
testthat::test_that("test circuit data cleaning, no cleaning required", {
    ...
    expected_circuit_details_cleaned <- load_test_file(
        circuit_details_path_name_clean
    )
    ...
    dp$run_data_cleaning_loop(max_chunk_size=)
    ...
    testthat::expect_equal(output_circuit_details, expected_circuit_details)
    testthat::expect_equal(
        output_circuit_details_cleaned,
        expected_circuit_details_cleaned
    )
})
