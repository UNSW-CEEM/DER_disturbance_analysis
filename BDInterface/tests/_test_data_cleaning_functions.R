...
test_that("Test calculation of sunrise and sunset times", {
  ...
  expect_equal(
      select(out, postcode, lon, lat, date, sunrise, sunset),
      expected_out,
      tolerance=0.01
  )
})
...
test_that("Test checking of day vs night energy", {
  ...
  energy_data <- mutate(
      energy_data,
      frac_day=energy_day/(energy_day+energy_night)
  )
  # Test output data
  c_id <- c("800", "4064", "6019", "6018", "1000")
  con_type <- c(
...
test_that("Test grouping of site data",{
  ...
  # Call processing function
  processed_site_details <- group_site_details_to_one_row_per_site(
      test_site_details
  )
  # Test the answer matches the expected answer
...
test_that("Test check_for_peak_power_greater_than_dc_capacity", {
  ...
  # Call processing function
  processed_site_details <- check_for_peak_power_greater_than_dc_capacity(
      test_site_details
  )
  # Test the answer matches the expected answer
  expect_equal(processed_site_details, expected_answer)
...
test_that("Test check_ac_capacity_less_than_peak_power", {
  ...
  # Call processing function
  processed_site_details <- check_ac_capacity_less_than_peak_power(
      test_site_details
  )
  # Test the answer matches the expected answer
  expect_equal(processed_site_details, expected_answer)
