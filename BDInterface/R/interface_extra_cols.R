...
DBInterface <- R6::R6Class("DBInterface",
  public = list(
    ...
    check_tables_have_expected_columns = function() {
      self$check_table_has_expected_columns('timeseries', c("ts", "c_id", "d", "d_key", "e", "f", "v")) #cols 
    ...
    build_database = function(timeseries, circuit_details, site_details, check_disk_space=TRUE, 
      ...
      RSQLite::dbExecute(con, "CREATE TABLE timeseries(
                         ...
                         v REAL,
                         f REAL,
                         PRIMARY KEY (ts, c_id, d_key))") #cols
    ...
    filter_out_unchanged_records = function(time_series){
      ...
      time_series <- select(time_series, ts, c_id, d_key, d, e, v, f) #cols
      return(time_series)
    }, 
    ...
    update_timeseries_table_in_database = function(time_series){
      sqldf::read.csv.sql(
        sql = "REPLACE INTO timeseries SELECT ts, c_id, d_key, e, v, f, d FROM time_series", #cols
        dbname = self$db_path_name)
    },
