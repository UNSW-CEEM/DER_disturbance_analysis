    ...
    tabPanel("Main", fluid = TRUE,
      sidebarLayout(
        sidebarPanel(id = "side_panel",
          ...
          textInput("settings_file", "Select JSON file for loading inputs (optional).", value = ""),
          ...
          uiOutput("duration"),
          HTML("<br><br>"),
          textInput("frequency_data", "Frequency data file", value = ""),
          ...
          materialSwitch(
            ...
            label = strong("Reconnection Compliance:"),
            status = "primary",
            value = FALSE
          ),
          ...
          radioButtons(
            "confidence_category",
            label = strong("Grouping category to calculate confidence interval for. Must be a Grouping Category"),
            choices = list(
              "none",
              "response_category",
              "compliance_status",
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          numericInput(
            ...
            label = strong(
              "Response time, window length for systems to be considered Non Compliant Responding, in seconds."
            ),
            value = 120
          ),
          ...
          numericInput(
            ...
            label = strong(
              paste0(
                "End buffer, allowed time for system ending response early, in seconds. ",
          ...
          numericInput(
            ...
            label = strong(
              paste0(
                "Pre-event UFLS Window: The time window before the event used to determine if the connection with a ",
                "device is stable enough to determine its UFLS status, in minutes."
              )
            ),
            value = 5
          ),
          numericInput(
            ...
            label = strong(
              paste0(
                "The fraction of the Pre-event UFLS Window that needs to be sampled for the connection with a device ",
                "to be considered stable enough to determine its UFLS status."
              )
            ),
            value = 0.6,
            max = 1,
          ...
          materialSwitch(
            "exclude_solar_edge",
            label = strong("Exclude solar edge from reconnection summary."),
            status = "primary",
            value = FALSE
          ),
          materialSwitch(
            "exclude_islanded_circuits",
            label = strong("Exclude islanded circuits from figures and results"),
            status = "primary",
            value = TRUE
          ),
    ...
    if (length(errors$errors) > 0) {
      ...
      output$sites <- renderUI({
        selectizeInput(
          ...
          choices = sort(unique(v$site_details$site_id)),
          multiple = TRUE
        )
      })
      output$circuits <- renderUI({
        selectizeInput(
          ...
          choices = sort(unique(v$circuit_details$c_id)),
          multiple = TRUE
        )
      })
      ...
      output$reconnection_compliance <- renderUI({
        checkboxGroupButtons(
          inputId = "reconnection_compliance",
          label = strong("Reconnection Compliance"),
          choices = list("Compliant", "Non Compliant", "Unsure", "Cannot be set", NA),
          selected = list("Compliant", "Non Compliant", "Unsure", "Cannot be set", NA),
          justified = TRUE,
          status = "primary",
          individual = TRUE,
          checkIcon = list(yes = icon("ok", lib = "glyphicon"), no = icon("remove", lib = "glyphicon")),
      ...
      output$window_length <- renderUI({
        numericInput(
          "window_length",
          label = strong("Set window length (min), Only data in this window is used for response analysis."),
          value = 5,
          min = 1,
          max = 100,
          step = 1
      ...
      output$post_event_ufls_window_length <- renderUI({
        numericInput(
          ...
          label = strong("Set post event UFLS window length (min)"),
          value = 5,
          min = 1,
          max = 100,
          step = 1
    ...
    if ((sum(v$sample_count_table$sample_count)<1000 & no_grouping) |
      ...
      if (length(v$combined_data_f$ts) > 0) {
        ...
        output$save_ideal_response_downsampled <- renderUI({
          shinySaveButton(
            "save_ideal_response_downsampled",
            "Save downsampled response",
            "Choose directory for report files ...",
            filetype = list(xlsx = "csv")
          )
        })
        ...
        output$save_ideal_response_downsampled_2020 <- renderUI({
          shinySaveButton(
            "save_ideal_response_downsampled_2020",
            "Save downsampled response 2020",
            "Choose directory for report files ...",
            filetype = list(xlsx = "csv")
          )
        })
        output$save_manufacturer_disconnection_summary <- renderUI({
          shinySaveButton(
            "save_manufacturer_disconnection_summary",
            "Save manufacturer disconnection summary",
            "Choose directory for report files ...",
            filetype = list(xlsx = "csv")
          )
        })
        ...
        output$save_upscaled_disconnection_summary <- renderUI({
          shinySaveButton(
            "save_upscaled_disconnection_summary",
            "Save upscaled disconnection summary",
            "Choose directory for report files ...",
            filetype = list(xlsx = "csv")
          )
        })
        ...
        output$save_voltage_excursion_summary <- renderUI({
          shinySaveButton(
            "save_voltage_excursion_summary",
        ...
        if (dim(v$ideal_response_to_plot)[1] > 0) {
          output$NormPower <- renderPlotly({
            plot_ly(v$agg_norm_power, x = ~Time, y = ~c_id_norm_power, color = ~series, type = "scattergl") %>%
              ...
              add_trace(
                x = ~v$ideal_response_to_plot_2020$ts,
                y = ~v$ideal_response_to_plot_2020$norm_power,
                name = "Ideal Response 2020",
                mode = "markers",
                inherit = FALSE
              ) %>%
              add_trace(
              ...
              layout(
                yaxis = list(
                  title = "Circuit power normalised to value of pre-event interval, \n aggregated by averaging"
                )
              )
          })
        } else {
        ...
        output$ResponseCount <- renderPlotly({
          plot_ly(v$response_count, x = ~series_x, y = ~sample_count, color = ~series_y, type = "bar") %>%
            layout(
              yaxis = list(title = "Fraction of circuits \n (denominator is count post filtering)"),
        ...
        output$ZoneCount <- renderPlotly({
          plot_ly(
            ...
            layout(
              yaxis = list(title = "Fraction of zone circuits \n (denominator is count post filtering)"),
        ...
        output$distance_response <- renderPlotly({
          plot_ly(v$distance_response, x = ~distance, y = ~percentage, color = ~series, type = "scattergl") %>%
            layout(
              yaxis = list(
                title = "Cumulative disconnects / Cumulative circuits \n (Includes response categories 3 and 4)"
              ),
        ...
        output$map <- renderPlotly({plot_geo(v$geo_data, lat = ~lat, lon = ~lon, color = ~percentage_disconnect) %>%
            ...
            layout(
              annotations = list(
                x = 1,
                y = -0.1,
                text = "Note: pecentage disconnects includes categories 3 and 4.",
                showarrow = F,
                xref = "paper",
                yref = "paper",
                xanchor = "right",
        ...
        v$reconnection_profile <- create_reconnection_profile(
          pre_event_interval(),
          ramp_length_minutes = 6,
