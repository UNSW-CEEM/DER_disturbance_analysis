...
test_that("Test grouping of site data",{
  ...
  model <- c("1", "2", "3")
  test_site_details <- data.frame(site_id, s_state, pv_installation_year_month, ac, dc,
                                  ac_old, dc_old, ac_dc_ratio, manufacturer,
                                  model, s_postcode, stringsAsFactors = FALSE)
  # Test output data
  site_id <- c(101, 300)
  s_state <- c("NSW", "SA")
  ...
  model <- c("1 2", "3")
  rows_grouped <- c(2, 1)
  expected_answer <- data.frame(site_id, s_state, pv_installation_year_month, sum_ac, sum_dc,
                                sum_ac_old, sum_dc_old, ac_dc_ratio, manufacturer,
                                model, s_postcode, rows_grouped, stringsAsFactors = FALSE)
  # Call processing function
